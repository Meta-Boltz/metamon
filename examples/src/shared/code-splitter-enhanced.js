/**
 * Enhanced Code Splitter for Ultra-Modern MTM Router
...
export class CodeSplitter {
  ...
  async loadChunk(importFn, options = {}) {
    ...
    try {
      ...
      // Create a load function that will be passed to the retry queue
      const loadFunction = async () => {
        return this.performChunkLoad(importFn, options, chunkId);
      };

      // Add to retry queue with priority based on options
      const priority = options.priority === 'high' ? 10 :
        options.priority === 'normal' ? 5 : 0;

      // Create a promise for the loading operation
      const loadPromise = this.retryQueue.add(
        loadFunction,
        [],
        {
          priority,
          ...
          onRetry: (count, error, delay) => {
            console.log(`🔄 Retrying chunk ${chunkId} (attempt ${count}/${this.options.maxRetries}) after ${delay}ms`);

            if (options.onRetry) {
              options.onRetry(count, error, chunkId);
            }
          }
        }
      );

      // Store the promise in the loading chunks map
      ...
      // If this was a retry success, update stats
      if (result.__retried) {
        this.loadStats.retrySuccesses++;
      }
  ...
  async performChunkLoad(importFn, options, chunkId) {
    ...
    try {
      ...
      console.warn('Safe assignment utility not available, falling back to direct assignment');
      // Fallback implementation if the utility can't be imported
      safeAssignUtil = (obj, prop, value) => {
        try {
          obj[prop] = value;
          return obj;
        } catch (e) {
          console.warn(`Failed to assign property ${prop}:`, e);
          return obj;
        }
      };
    ...
    try {
      ...
      try {
        ...
        if (signal.aborted) {
          throw new ChunkTimeoutError(
            `Chunk load timed out after ${chunkTimeout}ms`,
            {
              chunkId,
              filePath,
      ...
      if (module) {
        ...
        // Mark the module as having been retried if this isn't the first attempt
        if (options.__retryCount > 0) {
          module = safeAssignUtil(module, '__retried', true);
          module = safeAssignUtil(module, '__retryCount', options.__retryCount);
        }
      }
  ...
  getLoadStats() {
    return {
      ...
      cacheHitRate: this.loadStats.totalLoads > 0
        ...
        : 0,
      retrySuccessRate: this.loadStats.retryAttempts > 0
        ? (this.loadStats.retrySuccesses / this.loadStats.retryAttempts) * 100
        : 0,
      loadedChunks: this.loadedChunks.size,
      queuedPreloads: this.preloadQueue.length,
      activeLoads: this.loadingQueue.length,
      retryQueueState: this.retryQueue.getState()
    };
  }
...
/**
 * Re-export error classes from the chunk-error module
 */
export {
  ChunkError,
  ChunkNetworkError,
