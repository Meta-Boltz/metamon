 ...
 * 
 * This transformer addresses the chunk loading issue by ensuring that the generated code
 * is compatible with the chunk loader and avoids creating getter-only properties.
 */

import TemplateParser from './template-parser.js';

export class MTMTransformer {
  constructor() {
  ...
  transform(code, framework = 'react', options = {}) {
    ...
    try {
      ...
      // Add chunk loader compatibility wrapper
      if (opts.chunkCompatMode === 'safe') {
        transformedCode = this.addChunkLoaderCompatibility(transformedCode, framework);
      }
  ...
  /**
   * Add chunk loader compatibility wrapper to the transformed code
   * This ensures that the exported module uses writable properties instead of getter-only properties
   * 
   * @param {string} code - The transformed code
   * @param {string} framework - The target framework
   * @returns {string} - The code with chunk loader compatibility
   */
  addChunkLoaderCompatibility(code, framework) {
    // Add import for safe-assign utility
    const safeAssignImport = "import { safeAssign, safeAssignAll } from '../../../packages/core/src/utils/safe-assign.js';\n";

    // Add chunk compatibility wrapper
    const chunkCompatWrapper = `
// Chunk loader compatibility wrapper
const originalModule = module.exports;

// Create a new module exports object with writable properties
const safeExports = {};

// Copy all properties from the original module exports
Object.keys(originalModule).forEach(key => {
  // Use regular assignment for primitive values
  if (
    typeof originalModule[key] !== 'object' || 
    originalModule[key] === null ||
    Array.isArray(originalModule[key])
  ) {
    safeExports[key] = originalModule[key];
  } else {
    // For objects, create a new object to avoid getter-only properties
    safeExports[key] = { ...originalModule[key] };
  }
});

// Add metadata about the safe wrapper
safeExports.__mtm = {
  safeWrapper: true,
  framework: '${framework}',
  timestamp: new Date().toISOString(),
  originalKeys: Object.keys(originalModule)
};

// Replace module.exports with the safe version
module.exports = safeExports;
`;

    // For ESM format, we need a different approach
    const esmCompatWrapper = `
// Chunk loader compatibility wrapper for ESM
const __mtmSafeExports = {
  __mtm: {
    safeWrapper: true,
    framework: '${framework}',
    timestamp: new Date().toISOString()
...
// Create safe versions of all exports
export const __mtmEnsureSafeExports = () => {
  // This function will be called by the chunk loader to ensure safe exports
  return __mtmSafeExports;
};
`;

    // Check if the code uses ESM or CJS
    const isESM = code.includes('export default') || code.includes('export const') || code.includes('export function');

    if (isESM) {
      // For ESM, add the import at the top and the wrapper at the bottom
      return safeAssignImport + code + esmCompatWrapper;
    } else {
      // For CJS, add the import at the top and the wrapper at the bottom
      return safeAssignImport + code + chunkCompatWrapper;
    }
  }
...
export const __mtmMetadata = {
  ...
  framework: 'react',
  timestamp: new Date().toISOString(),
  hasTemplate: ${Boolean(parsed.template)},
  variableCount: ${parsed.variables.size},
  functionCount: ${parsed.functions.size}
};
`;
...
// Add metadata for chunk loader compatibility
const __mtmMetadata = {
  componentName: '${this.getComponentName(parsed)}',
  framework: 'vue',
  timestamp: new Date().toISOString(),
  hasTemplate: ${Boolean(parsed.template)},
  variableCount: ${parsed.variables.size},
...
// Export metadata for chunk loader compatibility
defineExpose({ __mtmMetadata });
`;
