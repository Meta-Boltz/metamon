// Fixed Ultra-Modern MTM Compiler
const fs = require('fs');
const path = require('path');

class FixedUltraModernCompiler {
  compile(inputFile) {
...
<body>
  ...
  processTemplate(template) {
    // Step 1: Handle event handlers FIRST (before variable interpolation)
    let processed = template;

    // Replace click={$func} with data-click="$func"
    processed = processed.replace(/click=\\{(\\$\\w+)\\}/g, 'data-click="$1"');

    // Step 2: Handle conditional rendering
...
// Step 3: Handle variable interpolation LAST
...
function ${componentName}() {
  // Variables
${this.generateVariables(parsed.variables)}

  // Functions  
${this.generateFunctions(parsed.functions, parsed.variables)}
...
${this.generateBindingUpdates(parsed.variables)}
    ...
    container.querySelectorAll('[data-if]').forEach(el => {
      ...
      try {
        const evalStr = condition${this.generateConditionReplacements(parsed.variables)};
...
${this.generateSubscriptions(parsed.variables)}
  
  // Bind events
${this.generateEventBindings(parsed.functions)}
}
...
generateFunctions(functions, variables) {
...
generateBindingUpdates(variables) {
  return variables.map(variable =>
    `    container.querySelectorAll('[data-bind="$${variable.name}"]').forEach(el => {
      el.textContent = $${variable.name}.value;
    });`
  ).join('\n');
}

generateConditionReplacements(variables) {
  let replacements = '';
  variables.forEach(variable => {
    replacements += `.replace(/\\\\$${variable.name}/g, '$${variable.name}.value')`;
  });
  return replacements;
}

generateSubscriptions(variables) {
  return variables.map(variable =>
    `  $${variable.name}.subscribe(() => updateAll());`
  ).join('\n');
}

generateEventBindings(functions) {
  return functions.map(func =>
    `  container.querySelectorAll('[data-click="$${func.name}"]').forEach(el => {
...
module.exports = { FixedUltraModernCompiler };