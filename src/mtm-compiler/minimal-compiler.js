// Minimal MTM Compiler - Handles ultra-minimal syntax without function wrappers
const fs = require('fs');
const path = require('path');

class MinimalMTMCompiler {
  compile(inputFile) {
  ...
  extractComponentName(filename) {
    ...
    return baseName.split('.')[0].replace(/[-_]/g, '').replace(/^\w/, c => c.toUpperCase()) + 'Component';
  }
  ...
  parseSource(source) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // Parse metadata (route, title, description)
...
// Ultra-Minimal Component: ${componentName}
...
document.addEventListener('DOMContentLoaded', () => {
  ${componentName}();
  console.log('🔮 Ultra-Minimal MTM Component loaded:', pageMetadata);
});`;
  }
...
module.exports = { MinimalMTMCompiler };