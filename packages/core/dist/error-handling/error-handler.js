...
export class ErrorHandler {
    ...
    provideSuggestions(error) {
        ...
        switch (error.type) {
            case 'syntax':
                suggestions.push('Use a linter or formatter for your target framework', 'Check the framework documentation for syntax requirements');
                break;
            case 'frontmatter':
                suggestions.push('Validate YAML syntax using an online YAML validator', 'Check the Metamon documentation for frontmatter schema');
                break;
            case 'framework':
                suggestions.push('Ensure all framework dependencies are installed', 'Check framework version compatibility');
                break;
            case 'runtime':
                suggestions.push('Check browser console for additional runtime errors', 'Verify component lifecycle and cleanup logic');
                break;
        }
        // Add file-specific suggestions
        if (error.file.includes('.mtm')) {
            suggestions.push('Verify .mtm file structure and frontmatter format', 'Check that target framework matches component code');
        }
     ...
     */
    createFromError(error, type, file, context) {
    ...
    enhanceError(error) {
        ...
        return new CompilationError(error.type, error.message, error.file, {
            line: error.line,
            column: error.column,
