...
export class MTMSourceMapGenerator {
    constructor(originalFile, originalContent, generatedFile) {
     ...
     */
    addMapping(generatedLine, generatedColumn, originalLine, originalColumn, originalFile, name) {
     ...
     */
    addGeneratedLine(generatedCode, originalLine, originalColumn = 0, originalFile) {
        const generatedLineNumber = this.generatedLines.length + 1;
        this.generatedLines.push(generatedCode);
        this.addMapping(generatedLineNumber, 0, originalLine, originalColumn, originalFile);
    }
    /**
     ...
     */
    static createForMTMCompilation(originalFile, originalContent, generatedCode, frontmatterLineCount = 0) {
        ...
        generatedLines.forEach((generatedLine, index) => {
            ...
            if (originalLineIndex < originalLines.length) {
                generator.addMapping(index + 1, 0, originalLineIndex + 1, 0, originalFile);
            }
        });
        return generator;
    }
     ...
     */
    static async findOriginalLocation(sourceMapContent, generatedLine, generatedColumn) {
        try {
