...
export class MTMParser {
    static parse(content, filePath) {
        try {
            ...
            if (!frontmatterMatch) {
                throw CompilationError.frontmatter('Invalid .mtm file format. Expected frontmatter section with --- delimiters.', filePath);
            }
            ...
            try {
                ...
                throw CompilationError.frontmatter(`Invalid YAML in frontmatter: ${yamlError instanceof Error ? yamlError.message : 'Unknown error'}`, filePath);
            }
            ...
            throw CompilationError.syntax(`Failed to parse .mtm file: ${error instanceof Error ? error.message : 'Unknown error'}`, filePath);
        }
    }
    static validateFrontmatter(frontmatter, filePath) {
        const validTargets = ['reactjs', 'vue', 'solid', 'svelte'];
        if (!frontmatter.target) {
            throw CompilationError.frontmatter('Missing required "target" field in frontmatter', filePath);
        }
        if (!validTargets.includes(frontmatter.target)) {
            throw CompilationError.frontmatter(`Invalid target "${frontmatter.target}". Must be one of: ${validTargets.join(', ')}`, filePath, [`Use one of: ${validTargets.join(', ')}`]);
        }
        ...
        if (frontmatter.channels) {
            for (const channel of frontmatter.channels) {
                if (!channel.event || !channel.emit) {
                    throw CompilationError.frontmatter('Channel configuration must have both "event" and "emit" fields', filePath, ['Example: { event: "user-action", emit: "onUserAction" }']);
                }
            }
        }
    }
...
export class MTMCompiler {
    ...
    compile(mtmFile) {
        ...
        if (!adapter) {
            throw CompilationError.framework(`No adapter found for target framework: ${mtmFile.frontmatter.target}`, mtmFile.filePath, mtmFile.frontmatter.target);
        }
        try {
            return adapter.compile(mtmFile);
        }
        catch (error) {
            throw CompilationError.framework(`Compilation failed for ${mtmFile.frontmatter.target}: ${error instanceof Error ? error.message : 'Unknown error'}`, mtmFile.filePath, mtmFile.frontmatter.target, error instanceof Error ? error : undefined);
        }
    }
}
/**
...
class FrameworkAdapter {
    ...
    generateEventHandlers(channels = []) {
  ...
  const ${channel.emit} = (data) => {
    ...
    convertTemplateCall(content, framework) {
...
${processedContent}${eventHandlers}`;
    ...
    compileTemplate(template, framework) {
