...
import { BundleInfo, BundleResult, ModuleBundlerConfig } from './types/module-bundler.js';
...
export class PerformanceTestRunner {
   ...
   * Test tree-shaking performance
   */
  private async testTreeShakingPerformance(): Promise<PerformanceTestResult> {
    const times: number[] = [];
   ...
   * Test bundle analysis performance
   */
  private async testBundleAnalysisPerformance(): Promise<PerformanceTestResult> {
    const times: number[] = [];
    const memoryUsages: number[] = [];
    ...
    return this.calculateTestResult('Bundle Analysis', times, memoryUsages, {
      ...
      totalBundleSize: bundleResult.bundles.reduce((sum: number, b: any) => sum + b.size, 0)
    });
  }
   ...
   * Test production optimization performance
   */
  private async testProductionOptimizationPerformance(): Promise<PerformanceTestResult> {
    const times: number[] = [];
    const memoryUsages: number[] = [];
    
    const config: ProductionOptimizationConfig = {
      minify: {
        enabled: true,
        removeComments: true,
        removeConsole: true,
   ...
   * Test module bundling performance
   */
  private async testModuleBundlingPerformance(): Promise<PerformanceTestResult> {
    const times: number[] = [];
    const memoryUsages: number[] = [];
    
    const bundler = new MTMModuleBundler();
    const config: ModuleBundlerConfig = {
   ...
   * Test end-to-end build performance
   */
  private async testEndToEndBuildPerformance(): Promise<PerformanceTestResult> {
    const times: number[] = [];
    const memoryUsages: number[] = [];
   ...
   */
  private generateTestBundle(sizeBytes: number): string {
    const baseCode = `
...
`;
   ...
   */
  private generateTestDependencyGraph(framework: string): any {
    const files = new Map();
   ...
   */
  private generateTestBundleResult(): BundleResult {
    const bundles: BundleInfo[] = this.config.bundleSizes.map((size, index) => ({
   ...
   */
  private generateOverview(results: PerformanceTestResult[]): {
    totalTests: number;
    totalTime: number;
    averageTime: number;
    fastestTest: string;
   ...
   */
  private generateComparison(results: PerformanceTestResult[]): {
    baseline: string;
    improvements: Array<{ test: string; improvement: number; percentage: number }>;
  } {
    // Use the slowest test as baseline
    const baseline = results.reduce((slowest, current) => 
   ...
   */
  private async saveResults(results: PerformanceTestSuiteResult): Promise<void> {
   ...
   */
  private generateMarkdownReport(results: PerformanceTestSuiteResult): string {
...
${results.comparison.improvements.map(imp => 
  `- **${imp.test}:** ${imp.improvement.toFixed(2)}ms faster (${imp.percentage.toFixed(1)}% improvement)`
).join('\n')}
