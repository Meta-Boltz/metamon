...
import { ReactAdapter, VueAdapter, SolidAdapter, SvelteAdapter } from '@metamon/adapters';
 ...
 */
export function metamon(options = {}) {
    const { root = 'src', pagesDir = 'pages', componentsDir = 'components', hmr = true, sourceMaps, adapters: customAdapters = {}, optimization = {} } = options;
    ...
    let productionOptimizer;
    let bundleAnalyzer;
    /**
     * Compile a .mtm file to framework-specific code
     */
    async function compileMTMFile(filePath) {
        try {
        ...
        async generateBundle(options, bundle) {
            ...
            if (mtmChunks.length > 0 && !server) {
                ...
                try {
                    ...
                    const bundleInfos = await Promise.all(mtmChunks.map(async (fileName) => {
                        const chunk = bundle[fileName];
                        if (chunk.type === 'chunk') {
                            return {
                                filePath: fileName,
                    ...
                    const validBundleInfos = bundleInfos.filter(Boolean);
