...
export class ReactAdapter {
    ...
    compile(mtmFile) {
        try {
            ...
            throw CompilationError.framework(error instanceof Error ? error.message : 'Unknown compilation error', mtmFile.filePath, 'React', error instanceof Error ? error : undefined);
        }
    }
    /**
...
export function useMetamonSignal(key, initialValue) {
  return useSignal(initialValue, key);
    ...
    setupPubSubIntegration(channels) {
        ...
        const eventHandlers = channels.map(channel => `const ${channel.emit} = useCallback((payload) => {
    pubSubSystem.emit('${channel.event}', payload);
  }, []);`).join('\n  ');
        const subscriptions = channels.map(channel => `useEffect(() => {
...
${subscriptions}`;
     ...
     */
    generateReactImports(userImports, channels) {
        const imports = ['import React from \'react\';'];
