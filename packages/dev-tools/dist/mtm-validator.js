export class MTMValidator {
    constructor() {
        this.supportedFrameworks = ['reactjs', 'vue', 'solid', 'svelte'];
        this.requiredChannelFields = ['event', 'emit'];
    }
    validate(mtmFile) {
        const errors = [];
    ...
    validateFrontmatter(frontmatter) {
        ...
        if (frontmatter.channels) {
            if (!Array.isArray(frontmatter.channels)) {
                ...
                frontmatter.channels.forEach((channel, index) => {
    ...
    validateContent(content, target) {
        const errors = [];
