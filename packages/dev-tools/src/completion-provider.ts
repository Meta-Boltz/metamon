...
export class MTMCompletionProvider {
  ...
  private provideFrontmatterCompletions(yamlPath: string[], text: string, position: Position): CompletionItem[] {
  ...
  private provideContentCompletions(text: string, position: Position): CompletionItem[] {
    const completions: CompletionItem[] = [];
  ...
  private getFrameworkSpecificCompletions(target: string): CompletionItem[] {
    const completions: CompletionItem[] = [];
  ...
  private getMetamonCompletions(): CompletionItem[] {
    return [
      {
  ...
  private getFrameworkDocumentation(framework: string): string {
    const docs = {
    ...
    return docs[framework as keyof typeof docs] || '';
  }
